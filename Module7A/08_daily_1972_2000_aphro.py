...
dateBegin = datetime(year=1972, month=1, day=1)
...
# %% [markdown]
## Remove old files 
outDir = "../../3_RSData/1_Rasters/GeneratedRasters/01_Daily/03_1972_2000_APHRO/"
fresh(outDir)
...
XAPHRO['elev'] = elev.flatten()  # add info from above files to XAPHRO
XAPHRO['aspect'] = aspect.flatten()
XAPHRO['slope'] = slope.flatten()
row, col = np.zeros((66,70)), np.zeros((66,70))  # add (row, col) to XAPHRO
for a in range(66):
    for b in range(70):
        row[a,b], col[a,b] = a,b
XAPHRO['row'] = row.flatten()
XAPHRO['col'] = col.flatten()  # XAPHRO prepared 

# [2] prepare 'X' for station data 
XSTN = stnKey.iloc[:,6:]
XSTN.rename(columns={'srtm':'elev'}, inplace=True)
XSTN = XSTN[XAPHRO.columns]

# [3] prepare 'X' for BKGN data  
XBKGN = XAPHRO.copy()

# [4] append them  
X = pd.concat([XAPHRO, XSTN, XBKGN])
...
## Make maps [contd.]
# [2] prepare 'y'
# [2.1] prepare 'y' from APHRO data (also convert to mm/day from kg/m^2/s)
for i in td(range(len(rasAPHRODaily)), desc='Making maps'):
    yAPHRO = (rio.open(rasAPHRODaily[i]).read(1)).flatten()
    fileName = rasAPHRODaily[i].split('/')[-1]  # filename extracted from path
    ...
    yBKGN = rio.open("../../3_RSData/1_Rasters/2_BKGN/07_DailyBasedOnAPHRO/{}".format(fileName)).read(1).flatten()

    # [2.4] append them  
    y = pd.DataFrame(np.concatenate([yAPHRO, ySTN, yBKGN]))
    ...
    # [4] predict results 
    yPred = regressor.predict(XAPHRO).reshape(66,70)

    # [5] save the output to tiff files
    referenceRas = gdal.Open(rasAPHRODaily[0])
    date = rasAPHRODaily[i].split('/')[-1].split('.')[0]  # date 
...
rasOut = sorted(glob.glob("../../3_RSData/1_Rasters/GeneratedRasters/01_Daily/03_1972_2000_APHRO/*.tif"))  # outfiles list 
...
outFileName = "../../2_ExcelFiles/02_SatDataAtSations/16_modelled_1972_2000_aphro.csv"
