...
for i in range(len(rasERA5Daily)):  # find the index of 20180531 file 
    if dateEnd in rasERA5Daily[i]:
        print(rasERA5Daily[i], 'at index', i)  # index of 20180531 file found 
rasERA5Daily = rasERA5Daily[:7822]
...
# %% [markdown]
## Remove old files 
outDir = "../../3_RSData/1_Rasters/GeneratedRasters/01_Daily/04_1972_2000_ERA5/"
fresh(outDir)
...
XERA5['elev'] = elev.flatten()  # add info from above files to XERA5
XERA5['aspect'] = aspect.flatten()
XERA5['slope'] = slope.flatten()
row, col = np.zeros((66,70)), np.zeros((66,70))  # add (row, col) to XERA5
for a in range(66):
    for b in range(70):
        row[a,b], col[a,b] = a,b
XERA5['row'] = row.flatten()
XERA5['col'] = col.flatten()  # XERA5 prepared 

# [2] prepare 'X' for station data 
XSTN = stnKey.iloc[:,6:]
XSTN.rename(columns={'srtm':'elev'}, inplace=True)
XSTN = XSTN[XERA5.columns]

# [3] prepare 'X' for BKGN data  
XBKGN = XERA5.copy()

# [4] append them  
X = pd.concat([XERA5, XSTN, XBKGN])
...
## Make maps [contd.]
# [2] prepare 'y'
# [2.1] prepare 'y' from ERA5 data 
for i in td(range(len(rasERA5Daily)), desc='Making maps'):
    yERA5 = (rio.open(rasERA5Daily[i]).read(1)).flatten()
    fileName = rasERA5Daily[i].split('/')[-1]  # filename extracted from path
    ...
    yBKGN = rio.open("../../3_RSData/1_Rasters/2_BKGN/08_DailyBasedOnERA5/{}".format(fileName)).read(1).flatten()

    # [2.4] append them  
    y = pd.DataFrame(np.concatenate([yERA5, ySTN, yBKGN]))
    ...
    # [4] predict results 
    yPred = regressor.predict(XERA5).reshape(66,70)
...
rasOut = sorted(glob.glob("../../3_RSData/1_Rasters/GeneratedRasters/01_Daily/04_1972_2000_ERA5/*.tif"))  # outfiles list 
...
outFileName = "../../2_ExcelFiles/02_SatDataAtSations/17_modelled_1972_2000_era5.csv"
