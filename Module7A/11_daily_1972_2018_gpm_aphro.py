# %% [markdown]
# # About
# > 1. This code prepares the modelled *daily precipitation* dataset for the duration 1972-2018  
...
# [2.3] set date as index and set a/c to stationKey
...
# %% [markdown]
## Read daily APHRODITE and GPM data
# [1] read files into two lists and merge the lists
rasters1 = sorted(glob.glob("../../3_RSData/1_Rasters/6_APHRODITE/V1101/daily/02_TiffResampled/*.tif"))
rasters2 = sorted(glob.glob("../../3_RSData/1_Rasters/1_GPM/daily_to_tiff_rsmpld/*.tif"))
rasters = rasters1 + rasters2

# [2] subset the list to station data duration
dateEnd = '2018_05_31'  # subset data till 2018 May 31
for i in range(len(rasters)):  # find the index of 20180531 file 
    if dateEnd in rasters[i]:
        print(rasters[i], 'at index', i)  # index of 20180531 file found 
rasters = rasters[:16953]  # subset done




# %% [markdown]
## Read daily BKGN data 
# [1] read files into two lists and merge the lists
rasBKGN1 = sorted(glob.glob("../../3_RSData/1_Rasters/2_BKGN/07_DailyBasedOnAPHRO/*.tif"))
rasBKGN2 = sorted(glob.glob("../../3_RSData/1_Rasters/2_BKGN/05_DailyBasedOnGPM/*.tif"))
rasBKGN = rasBKGN1 + rasBKGN2

# [2] subset the list to station data duration
dateEnd = '2018_05_31'  # subset data till 2018 May 31
for i in range(len(rasBKGN)):  # find the index of 20180531 file 
    if dateEnd in rasBKGN[i]:
        print(rasBKGN[i], 'at index', i)  # index of 20180531 file found 
rasBKGN = rasBKGN[:16953]  # subset done
...
# %% [markdown]
## Remove old files 
outDir = "../../3_RSData/1_Rasters/GeneratedRasters/01_Daily/06_1972_2018_GPM_APHRO/"
# fresh(outDir)
...
# [1] prepare 'X' for satellite data
XSAT = pd.DataFrame()
...
XSAT['elev'] = elev.flatten()  # add info from above files to X
XSAT['aspect'] = aspect.flatten()
XSAT['slope'] = slope.flatten()
row, col = np.zeros((66,70)), np.zeros((66,70))  # add (row, col) to X
for a in range(66):
    for b in range(70):
        row[a,b], col[a,b] = a,b
XSAT['row'] = row.flatten()
XSAT['col'] = col.flatten()  # X prepared 

# [2] prepare 'X' for station data 
XSTN = stnKey.iloc[:,6:]
XSTN.rename(columns={'srtm':'elev'}, inplace=True)
XSTN = XSTN[XSAT.columns]

# [3] prepare 'X' for BKGN data  
XBKGN = XSAT.copy()

# [4] append them  
X = pd.concat([XSAT, XSTN, XBKGN])
...
## Make maps [contd.]
# [2] prepare 'y'
# [2.1] prepare 'y' from satellite data
for i in td(range(len(rasters)), desc='Making maps'):
    ySAT = (rio.open(rasters[i]).read(1)).flatten()
    fileName = rasters[i].split('/')[-1]  # filename extracted from path
    ...
    yBKGN = rio.open(rasBKGN[i]).read(1).flatten()

    # [2.4] append them  
    y = pd.DataFrame(np.concatenate([ySAT, ySTN, yBKGN]))
    y.interpolate(method='linear', inplace=True)  # NaNs filled
    y.fillna(method='bfill', inplace=True)  # for the NaNs left by above line of code
    ...
    # [4] predict results 
    yPred = regressor.predict(XSAT).reshape(66,70)
...
rasOut = sorted(glob.glob(outDir + "*.tif"))  # outfiles list 
...
outFileName = "../../2_ExcelFiles/03_ModelledData/01_monthly_mean_1972_2018.csv"
