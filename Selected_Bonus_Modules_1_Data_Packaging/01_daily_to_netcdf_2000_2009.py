# %% [markdown]
# # About 
# This code is for the conversion of modelled daily tiff files to netcdf format for the bkgn period i.e. 2000-2009
...
# [1] read and subset files to bkgn duration
rasters = sorted(glob.glob("../../3_RSData/1_Rasters/GeneratedRasters/01_Daily/01_2000_2018_GPM/*.tif"))
dateEnd = '2009_12_31'  # subset data fill 2009 Dec 31
for i in range(len(rasters)):  # find the index of 20091231 file 
    if dateEnd in rasters[i]:
        print(rasters[i], 'at index', i)  # index of 20091231 file found 
rasters = rasters[:3501]  # subset done

# [2] set out directory
outDir = "../../3_RSData/1_Rasters/GeneratedRasters/02_DailyNetcdf/04_2000_2009_GPM/"
fresh(outDir)  

#[3.1] convert using rioxarray 
for i in td(range(len(rasters)), desc='Converting'):  
    ...
    # [3.2] set year, month and day variables
    year = int(date.split('-')[0])  
    month = int(date.split('-')[1])
    day = int(date.split('-')[2])

    # [3.3] reopen the saved nc file and change some properties
...
outName = outDir1 + "prcp_2000_2009.nc"  # outfile name set 
