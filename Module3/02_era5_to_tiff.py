# %% [markdown]
# # About
# This code converts ERA5 netcdf data to tiff format and then reprojects, resamples and clips it to study area's extent
...
# %% [markdown]
# # Read files
ds = xr.open_dataset("../../3_RSData/1_Rasters/4_ERA5/ERA5.nc")  
...
outDir = "../../3_RSData/1_Rasters/4_ERA5/monthly_means/01_netcdf/"  # results directory
months = ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC']  # months list
...
rasters = sorted(glob.glob("../../3_RSData/1_Rasters/4_ERA5/monthly_means/01_netcdf/*.nc"))  # read files 
outDir = "../../3_RSData/1_Rasters/4_ERA5/monthly_means/02_tiff/"
for i in td(range(0, len(rasters)), desc='Converting to tiff'):
    ...
    ds['tp_0001'].rio.to_raster(outName) # saved as tif



# %% [markdown]
# # Reproject, resample and clip 
rasters = sorted(glob.glob("../../3_RSData/1_Rasters/4_ERA5/monthly_means/02_tiff/*.tif"))
outDir = '../../3_RSData/1_Rasters/4_ERA5/monthly_means/03_resampled/'
